...
internal class ContentViewModelTest : BaseTest() {
    ...
    fun `Content data is invoked`() {
        ...
        viewModel.query(name, null, Sort.ASCENDING)
...
//        coVerify(exactly = 1) { useCase.invoke(any()) }
    ...
    fun `Drop content is invoked`() {
        ...
        coEvery {
            useCase.invoke(
                mockk {
                    ...
                    every { statement } returns "my_content"
                }
            )
        } returns mockk {
        ...
        coVerify(exactly = 1) { useCase.invoke(any()) }
//        assertEquals(viewModel.eventFlow.replayCache.first(), ContentEvent.Dropped())
    }
}
